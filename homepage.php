<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dressify Fashion Designing</title>
    <link rel="stylesheet" href="css/style.css">
   
       
</head>
<body>
    <div class="hero">

    <nav>
        <img src="images/logo.jpg" style="width:80px; height: 60px;">
        <div class="logo">Dressify</div>
        <ul>
            <li><a href="homepage.html">Home</a></a></li>
            <li><a href="categories.html">Categories</a></li>
            <li><a href="designerpage.html">Designer's Area</a></li>
            <li><a href="customerpage.html">Customer's Area</a></li>
            <li><a href="contactuspage.html">Contact Us</a></li>
            <li><a href="feedback.html">Feedback</a></li>
        </ul>
        <div class="account-cart">
            <a href="#"><a href="myaccountpage.html">My account</a></a>
        
        </div>
    </nav>
    <section>
    <div class="content">
            <h1>Make<br>New Friends</h1>
            <p>Explore our curated collection of stylish clothing and tailored to your unique taste</p>
            <a href="categories.html" class="btn" >Explore Now</a>
        </div>
            <img src="images/pic.jpg" class="feature-img ">
    </section>
</div>

</body>
</html>